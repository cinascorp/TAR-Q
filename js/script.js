...
'https://{a-c}.basemaps.cartocdn.com/{z}/{x}/{y}{r}.png',
'https://{a-c}.tile.openstreetmap.org/{z}/{x}/{y}.png',
];

const COLORS = {
  pvrtJ: getCssVar('--pvrt'),
...
const HELI_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="gray"><path d="M3 12h10a4 4 0 110 8H8a5 5 0 01-5-5v-3zm14-6h-4a1 1 0 110-2h9a1 1 0 110 2h-3v5h-2V6z"/></svg>')}`;

const AIRPLANE_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="azure"><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>')}`;

const AIRPLANE_SMALL_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill=""><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>')}`;

const AIRPLANE_PROP_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill=""><path d="M21 16v-2l-8-5V3.5c0-.83-.67-1.5-1.5-1.5S10 2.67 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z"/></svg>')}`;
            		
const FIGHTER_SVG = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path fill="red" d="M12 2l3 5h5l-4 4 2 9-6-4-6 4 2-9-4-4h5z"/></svg>')}`;
...
function updateDroneLegendIcons() {
    	const el = document.getElementById('legend-drone-menu');
      	if (!el) return;
      	const src = (currentMapLayer === 'dark') ? DRONE_SVG_BLACK : DRONE_SVG;
...
const baseLayer = new ol.layer.Tile({
            			source: new ol.source.XYZ({ url: TILE_URLS[0], attributions: '© ArcGIS, © OpenStreetMap' })
            		});
...
const popup = new ol.Overlay(  {element: popupEl, autoPan: {
...
const countpvrtEl = document.getElementById('countpvrt');
...
const trpEl = document.getElementById('trp');
...
function metersToFeet(m) { 
  return Math.round(Number(m) / 0.3048); }
...
function setGroupOptions(total) {
  const labels = makeGroupLabels(total || Math.max(26000, sortedHexes.length));
	groupSelect.innerHTML = '';
    labels.forEach((lab, idx) => {
    const opt = document.createElement('option');
    opt.value = lab;
    opt.textContent = `${lab} (${idx*26000+1}–${(idx+1)*26000})`;
...
async function fetchFR24(bounds, useProxy=true) {
  const params = new URLSearchParams({faa: '1', satellite: '1', mlat: '1', flarm: '1', adsb: '1', gnd: '1', air: '1', vehicles: '1', estimated: '1', maxage: '14400', gliders: '1', stats: '1',uav: '1',mil: '1',ladd: 1,
...
if (data.full_count) fullCount = Number(data.full_count) || 0;
    ...
    if (!Array.isArray(val) || val.length < 17) continue;
    const hex = val[0];
    const lat = Number(val[1]);
    const lon = Number(val[2]);
  	if (isNaN(lat) || isNaN(lon) || lat < -90 || lat > 90 || lon < -180 || lon > 180) continue;
    const heading = Number(val[3]);
  	const alt_ft = Number(val[4]);
  	const spd = Number(val[5]);
		const squawk = val[6];
    const acType = val[8];
		const registration = val[9];
  	const ts = Number(val[10]) * 1000;
  	const origin = val[11];
		const dest = val[12];
  	const flightNumber = val[13] || val[16];
  	const vertSpeed = Number(val[15]);
		const callsign = val[16];
  	const alt_m = feetToMeters(alt_ft);
    const spd_kmh = toKmH(spd);
  	
result.push({ hex, lat, lon, heading, alt_m, alt_ft, spd_kmh, squawk, acType, registration, origin, dest, flightNumber, callsign, vertSpeed, ts });	}
...
const pvrtRegPatterns = [ /^N[0-9A-Z]{1,5}$/i, /^[A-Z]{1,2}-[A-Z]{3,5}$/i, /^RA-\d{4,}$/i, /^9H-[A-Z]{3}$/i, /^CS-[A-Z]{3}$/i, /^OY-[A-Z]{3}$/i ];
...
function isDrone(f) {
    const spd = f.spd_kmh || 0;
  	const alt = f.alt_m || 0;
		const lowAlt = alt > 0 && alt <= 1200; 
  	const slow = spd > 0 && spd <= 120; 
...
function ispvrt(f) {
	...
	if (f.callsign && pvrtRegPatterns.some(r => r.test(f.callsign))) return true;
  
  if (f.registration && pvrtRegPatterns.some(r => r.test(f.registration))) return true;
  return false;	}
...
function estimatePassengerCapacity(f) {
  if (!f.acType) return 'unknown';
	...
	if (type.includes('B73') || type.includes('B74') || type.includes('B75') || type.includes('B76') || type.includes('B77') || type.includes('B78') || type.includes('A31') || type.includes('A32') || type.includes('A33') || type.includes('A34') || type.includes('A35') || type.includes('A38')) return 'large_passenger';
	return 'unknown';	}

function spectrumColorForAltitudeMeters(meters) {
  const t = normalize(meters || 0, 0, 17000); 
...
function getDroneIconColor(f) {
 
return currentMapLayer === 'dark' ? '#0101010' : '#fffffff'; }
...
function updateMarkersAndTracks() {
  ...
  const visibleSet = new Set(slice);
  const durationMs = autoUpdateSwitch && autoUpdateSwitch.checked ? 5000 : 3000;
  for (const hex of slice) {
                const f = flightsByHex.get(hex);
                if (!f) continue;
                if (animatedFeatures.has(hex)) {
                  const entry = animatedFeatures.get(hex);
                  entry.feature.set('data', f);
                  setFlightTarget(f, durationMs);
                  addToTrack(f);
                } else {
                  setFlightTarget(f, durationMs);
                }
              }
  for (const [hex, entry] of Array.from(animatedFeatures.entries())) {
  if (!visibleSet.has(hex)) {
  try { markerLayer.getSource().removeFeature(entry.feature); } catch(e) {}
    animatedFeatures.delete(hex);}}
    animatePositions();
    trackLayer.getSource().clear();
  for (const hex of selectedFlights) {
  const f = flightsByHex.get(hex);
    if (!f) continue;
      const segs = drawTrack(hex, categorize(f)); 
        if (segs) segs.forEach(sf => trackLayer.getSource().addFeature(sf));}};

            		
function updateInfoTables(allFlights) {
  const counts = { passenger: 0, commercial: 0, pvrt: 0, helicopter: 0, military: 0, drone: 0 };
  const unidentified = [];
    for (const f of allFlights) {
      const cat = categorize(f);
      	if (counts[cat] != null) counts[cat]++;
        	else counts.commercial++;
          	if (cat === 'drone' && !f.callsign && !f.registration) unidentified.push(f);
            			}
            			...
            			countpvrtEl.textContent = counts.pvrt;
                  ...
                  const tbody = document.querySelector('#droneByCountryTable tbody');
            			...
            			for (const country of countries) {
            				for (const f of mapCountryToRows.get(country)) {
            					const tr = document.createElement('tr');
            					const agl = (f.agl_m != null) ? f.agl_m : '—';
            					tr.innerHTML = `<td>${country}</td><td>${f.hex}</td><td>${f.lat.toFixed(2)}</td><td>${f.lon.toFixed(3)}</td><td>${agl}</td><td>${f.spd_kmh ?? '—'}</td>`;
            					tbody.appendChild(tr);
            				}
            			}
            		}
...
function viewGroupDetails(country, group) {
            			...
            			for (const f of groupFlights) {
            				const tr = document.createElement('tr');
            				const flightNumber = f.callsign || f.registration || f.hex;
...
map.on('singleclick', (evt) => {
    ...
    map.forEachFeatureAtPixel(evt.pixel, (feature, layer) => {
            				...
            				const title = d.callsign || d.registration || d.hex;
...
function makeFR24Link(f) {
  ...
  const idPart = f.hex;
  return `https://www.flightradar24.com/${f.callsign}/3d`;}
...
function drawEnhancedTrack(hex, category) {
            			...
            			trackFeature.setStyle(new ol.style.Style({ stroke: new ol.style.Stroke({ color: baseColor, width: 3, lineDash: [9, 6] }) }));
...
function startAutoUpdate() { 
  if (autoUpdateTimer) clearInterval(autoUpdateTimer); autoUpdateTimer = setInterval(refresh, 3000); }
...
function lerpAngle(aDeg, bDeg, t) {
              if (aDeg == null) return bDeg ?? 0;
              if (bDeg == null) return aDeg ?? 0;
...
function setFlightTarget(f, durationMs) {
              const hex = f.hex;
              const target = { lon: f.lon, lat: f.lat, heading: (f.heading != null ? f.heading : null) };
              if (animatedFeatures.has(hex)) {
                const entry = animatedFeatures.get(hex);
                if (!entry.from) {
                  try {
                    const coords = entry.feature.getGeometry().getCoordinates();
                    const [lon, lat] = ol.proj.toLonLat(coords);
                    entry.from = { lon, lat, heading: entry.to ? entry.to.heading : (f.heading || 0) };
                  } catch (e) {
                    entry.from = { lon: target.lon, lat: target.lat, heading: target.heading || 0 };
                  }
                }
                const now = Date.now();
                const elapsed = Math.max(0, now - (entry.startTs || now));
                const tNow = Math.min(1, (durationMs > 0) ? (elapsed / durationMs) : 1);
                const curLon = lerp(entry.from.lon, entry.to.lon, tNow);
                const curLat = lerp(entry.from.lat, entry.to.lat, tNow);
                const curHeading = lerpAngle(entry.from.heading ?? target.heading ?? 0, entry.to.heading ?? target.heading ?? 0, tNow);

                entry.from = { lon: curLon, lat: curLat, heading: curHeading };
                entry.to = target;
                entry.startTs = Date.now();
                entry.duration = durationMs;
              } else {
                const feat = featureForFlight(f);
                feat.setGeometry(new ol.geom.Point(ol.proj.fromLonLat([target.lon, target.lat])));
                markerLayer.getSource().addFeature(feat);
                animatedFeatures.set(hex, {
                  feature: feat,
                  from: { lon: target.lon, lat: target.lat, heading: target.heading ?? 0 },
                  to: target,
                  startTs: Date.now(),
                  duration: durationMs
                });
...
if (animationRunning) return;
              ...
              (function frame() {
                ...
                for (const [hex, entry] of animatedFeatures) {
                  const d = Math.max(1, entry.duration || durationMs);
                  const elapsed = now - (entry.startTs || now);
                  let t = Math.max(0, Math.min(1, elapsed / d));
                  const lon = lerp(entry.from.lon, entry.to.lon, t);
                  const lat = lerp(entry.from.lat, entry.to.lat, t);
                  const heading = lerpAngle(entry.from.heading ?? 0, entry.to.heading ?? 0, t);
                  entry.feature.getGeometry().setCoordinates(ol.proj.fromLonLat([lon, lat]));

                  try {
                    const st = entry.feature.getStyle();
                    if (st && st.getImage && typeof st.getImage === 'function') {
                      const img = st.getImage();
                      if (img && typeof img.setRotation === 'function') {
                        img.setRotation((heading || 0) * Math.PI / 180);

                        entry.feature.changed();
                      } else {
                        
                        entry.feature.setStyle(featureForFlight(entry.feature.get('data')).getStyle());
                      }
                    }
                  } catch (e) {
                  ...
                  if (t < 1) any = true;
                    ...
                    entry.from = { lon: entry.to.lon, lat: entry.to.lat, heading: entry.to.heading ?? 0 };
                    entry.startTs = now;
                    entry.duration = durationMs;
                  }
                }
...
async function getAltitudeColorForFeet(altFt) {
                if (altFt == null || isNaN(altFt)) altFt = 0;
                altFt = Math.max(0, Math.min(50000, Number(altFt)));
                if (altFt <= 7000) return '#FFA500';
                if (altFt <= 14000) return '#FFFF00';
                if (altFt <= 21000) return '#00FF00';
                if (altFt <= 28000) return '#0000FF';
                if (altFt <= 35000) return '#FF69B4';
                if (altFt <= 42000) return '#3300CC';
                if (altFt <= 49000) return '#957DAD';
                return
            }
            
async function fetchAdsbData() {
        try {
          const response = await fetch('https://cors.isomorphic-git.org/https://api.adsb.lol/v2/mil', {
            headers: {
...
async function fetchOpenSkyData() {
        ...
        try {
          const response = await fetch('https://cors.isomorphic-git.org/https://opensky-network.org/api/states/all', {
            headers: {
              'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
              'Accept': 'application/json'
            }
          ...
          return (data.states || []).map(flight => ({
            ...
            lon: flight[5],
            lat: flight[6],
            alt_baro: flight[7] || 'Unknown',
            velocity: flight[9] || 'Unknown',
            heading: flight[10] || 0,
            type: 'aircraft',
            hex: flight[0] || 'Unknown',
            squawk: flight[14] || 'Unknown',
            emergency: flight[15] || 'Unknown',
            baro_rate: flight[11] || 'Unknown',
            alt_geom: flight[13] || 'Unknown',
            nav_modes: flight[16] || [],
            nac_p: flight[17] || 'Unknown',
            nac_v: flight[18] || 'Unknown',
            sil: flight[19] || 'Unknown',
            sil_type: flight[20] || 'Unknown',
            gva: flight[21] || 'Unknown',
            sda: flight[22] || 'Unknown',
            alert: flight[23] || 'Unknown',
            spi: flight[24] || 'Unknown'
          }));
        } catch (error) {
          return [];
        }
...
async function fetchFlightradarData() {
        try {
          const response = await fetch('https://cors.isomorphic-git.org/https://data-cloud.flightradar24.com/zones/fcgi/feed.js', {
            headers: {
              'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
              'Accept': 'application/json'
            }
          ...
          for (const [key, value] of Object.entries(data)) {
            ...
            flights.push({
              ...
              lon: value[2],
              track: value[3] || 0,
              alt_baro: value[4] || 'Unknown',
              gs: value[5] || 'Unknown',
              flight: value[13] || 'Unknown',
              hex: value[0] || 'Unknown',
              type: 'civilian',
              model: value[8] || 'Unknown',
              r: value[9] || 'Unknown',
              category: value[11] || 'Unknown'
            });
          }
          return flights;
        } catch (error) {
...
function identifyFlightType(flight) {
        ...
        if (!flight.callsign && !flight.flight || flight.callsign === 'Unknown' && flight.flight === 'Unknown') {
          return 'unknown';
        }
...
function createMilitaryPopupContent(flight) {
        const flightradarLink = flight.hex && flight.flight !== 'Unknown' ? 
          `<a class="flightradar-link" href="https://www.flightradar24.com/${flight.hex}/${flight.flight}/3d" target="_blank">View 3D Path</a>` : '';
        
        const actionButtons = `
          <div style="margin-top: 10px; direction: ltr;">
            <button onclick="window.parent.militaryAction('track')" style="background: #27ae60; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Track</button>
            <button onclick="window.parent.militaryAction('intercept')" style="background: #f39c12; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Intercept</button>
            <button onclick="window.parent.militaryAction('jam')" style="background: #9b59b6; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Jam</button>
          </div>
        `;
        
        return `
          <b>Data source type: </b>${flight.type || 'Unknown'}<br>
          <b>Hex ID: </b>${flight.hex || 'Unknown'}<br>
          <b>Flight ID: </b>${flight.flight || flight.callsign || 'Unknown'}<br>
          <b>Registration number: </b>${flight.r || 'Unknown'}<br>
          <b>Aircraft type: </b>${flight.t || flight.model || 'Unknown'}<br>
          <b>Barometric altitude (feet): </b>${flight.alt_baro || 'Unknown'}<br>
          <b>Geometric altitude (feet): </b>${flight.alt_geom || 'Unknown'}<br>
          <b>Ground speed (knots): </b>${flight.gs || flight.velocity || 'Unknown'}<br>
          <b>Direction of movement (degrees): </b>${flight.track || flight.heading || 'Unknown'}<br>
          <b>Squawk code: </b>${flight.squawk || 'Unknown'}<br>
          <b>Emergency status: </b>${flight.emergency || 'Unknown'}<br>
          <b>Latitude: </b>${flight.lat || 'Unknown'}<br>
          <b>Longitude: </b>${flight.lon || 'Unknown'}<br>
          ${flightradarLink}
          ${actionButtons}
        `;
      }
...
function createCivilianPopupContent(flight) {
        const flightradarLink = flight.hex && flight.flight !== 'Unknown' ? 
          `<a class="flightradar-link" href="https://www.flightradar24.com/${flight.hex}/3d" target="_blank">View 3D and Path</a>` : '';
...
return `
          ...
          <b>Flight ID: </b>${flight.flight || flight.callsign || 'Unknown'}<br>
          <b>Registration number: </b>${flight.r || 'Unknown'}<br>
          <b>Aircraft type: </b>${flight.model || 'Unknown'}<br>
          <b>Barometric altitude (feet): </b>${flight.alt_baro || 'Unknown'}<br>
          <b>Ground speed (knots): </b>${flight.gs || flight.velocity || 'Unknown'}<br>
          <b>Direction of movement (degrees): </b>${flight.track || flight.heading || 'Unknown'}<br>
          <b>Latitude: </b>${flight.lat || 'Unknown'}<br>
...
const actionButtons = `<div style="margin-top: 10px; direction: ltr;"><button onclick="window.parent.militaryAction('track')" style="background: #27ae60; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Track</button><button onclick="window.parent.militaryAction('intercept')" style="background: #f39c12; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Intercept</button><button onclick="window.parent.militaryAction('jam')" style="background: #9b59b6; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Jam</button><button onclick="window.parent.militaryAction('destroy')" style="background: #e74c3c; color: white; border: none; padding: 5px 10px; margin: 2px; border-radius: 3px; cursor: pointer;">Destroy</button></div>`;
        
return `<b>Data source type: </b>${drone.type || 'Unknown'}<br>
          <b>Drone ID: </b>${drone.id || 'Unknown'}<br>
          <b>Model: </b>${drone.model || 'Unknown'}<br>
          <b>Latitude: </b>${drone.lat || 'Unknown'}<br>
          <b>Longitude: </b>${drone.lon || 'Unknown'}<br>
          <b>Barometric altitude (feet): </b>${drone.alt_baro || 'Unknown'}<br>
          <b>Ground speed (knots): </b>${drone.velocity || 'Unknown'}<br>
          <b>Direction of movement (degrees): </b>${drone.heading || 'Unknown'}<br>
          <b>Type: </b>Drone<br>${actionButtons}`;}
...
(function activateQuantumMode() {
              console.log("%c TAR-Q QUANTUM CORE INITIATED ", "background: #000; color: #00f3ff; font-size: 20px; border: 1px solid #00f3ff;");
             const originalFetch = window.fetch;
window.fetch = async function(url, options) {
        if(url.includes('flightradar') || url.includes('api')) {
            ...
            console.log(`%c[BLOB STREAM] Size: ${blobSize}b | Latency: ${latency.toFixed(1)}ms | Quantum Potential: HIGH`, "color: #0f0; font-family: monospace;");
          
            if(latency < 5 && blobSize > 1000) {
                 alert.log("%c⚠️ ANOMALY DETECTED (Possible Stealth Target) ⚠️", "color: red; font-weight: bold; blink;");
            }
    ...
    style.innerHTML = `.quantum-active {
    ...
    document.head.appendChild(style);
        ...
        if(mapDiv) mapDiv.classList.add('quantum-active');
    
   alert("TAR-Q QUANTUM MODE: ENGAGED\n\nA Cina's Research .");
